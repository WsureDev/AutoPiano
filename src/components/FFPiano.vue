<template>
  <div class="onepiano-app-wrap">
    <div class="trade-mark">
      ...
      <div class="trade-mark-txt">FF钢琴<span class="trade-mark-txt--en">FF Piano</span></div>
    </div>
...
import { Lyrics, Ffnotes, OBEvent } from 'config/'
...
export default {
  ...
  methods: {
    ...
    computeEleSize() {
      var wkey_width = $('.piano-key-wrap').width() / 22;
    ...
    bindKeyBoradEvent() {
      ...
      document.addEventListener('keydown', (e) => {
        ...
        if(vm.keydowns[e.key])
        {
          console.log('key is already down!',e.key);
          return ;
        }
        vm.keydowns[e.key] = e.keyCode;
        if (vm.DEV) console.log('keydown-key', e.key);
        // 按住空格键，则启用黑色按键
        if (keyCode == 32) {
...
.onepiano-app-wrap { width: 100%; min-height: 100%; position: relative; color: #000;
  ...
  .piano-wrap { width: 90%; margin: 20px auto; box-shadow: 2px 2px 10px 2px #acacac;
    ...
    .piano-key-wrap {
      ...
      .bkey-wrap2 {left: 0;}
      .bkey-wrap3 {left: 31.8%;}
      .bkey-wrap4 {left: 63.6%;}
